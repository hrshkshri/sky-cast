    ...
    <div className='weatherData'>
      <div className='currtemp'>
        <div className='tempAndLogo'>
          <div>
            <Image src={`/assets/${props.weather.icon}.svg`} width={200} height={200} alt='icon' />
          </div>
          <div>
      ...
      <div id='scrolledItem' className='forcastdata'>
        <div>
          ...
          <Image src={'https://bmcdn.nl/assets/weather-icons/v3.0/fill/svg/sunrise.svg'} width={100} height={100} alt='icon' />
          <p>{getTime(props.city.sunrise)}</p>
        </div>
        <div>
          <p>{props.lang ? 'HUMIDITY' : 'नमी'}</p>
          <Image src={'https://bmcdn.nl/assets/weather-icons/v3.0/fill/svg/humidity.svg'} width={100} height={100} alt='icon' />
          <p>{props.weatherData.humidity}&nbsp;mm</p>
        </div>
        <div>
          <p>{props.lang ? 'WIND' : 'हवा'}</p>
          <Image src={'https://bmcdn.nl/assets/weather-icons/v3.0/fill/svg/wind.svg'} width={100} height={100} alt='icon' />
          <p>{props.windData.speed}&nbsp;mph</p>
        </div>
        <div>
          <p>{props.lang ? 'PRESURE' : 'दबाव'}</p>
          <Image src={'https://bmcdn.nl/assets/weather-icons/v3.0/fill/svg/pressure-low.svg'} width={100} height={100} alt='icon' />
          <p>{props.weatherData.pressure}&nbsp;mb</p>
        </div>

        <div>
          <p>{props.lang ? 'SUNSET' : 'सूर्यास्त'}</p>
          <Image src={'https://bmcdn.nl/assets/weather-icons/v3.0/fill/svg/sunset.svg'} width={100} height={100} alt='icon' />
          <p>{getTime(props.city.sunset)}</p>
        </div>
      </div>
      <p className='copyright'>&copy;Harsh Keshari 2023</p>
